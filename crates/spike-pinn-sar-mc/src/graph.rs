//! Training and inference graphs for the high-D SAR PINN.
//! Pure data-MSE regression; same shape as `spike-pinn-sar` but with
//! input dim 10 instead of 1, and a wider MLP.
...
pub fn build_training_graph(batch: usize) -> TrainingGraph {
    let b = batch;
    let mut g = Graph::new("pinn_sar_mc_train");

    let x        = g.input("x",       s2(b, INPUT_DIM));
...
pub fn build_inference_graph(batch: usize) -> (Graph, Vec<ParamSpec>) {
    let mut g = Graph::new("pinn_sar_mc_infer");
    let x = g.input("x_inf", s2(batch, INPUT_DIM));
    let mlp = Mlp::new(&mut g, "m", ARCH_DIMS, ACT);
